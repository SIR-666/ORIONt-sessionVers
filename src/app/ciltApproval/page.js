...
const ConfirmModal = ({ show, onClose, onConfirm, item, type = "approve", userRole }) => {
...
const PerformaDetailContent = React.memo(({ item }) => {
    ...
    useEffect(() => {
        ...
        fetchPerformaData();
    }, [item.plant, item.line, item.shift, item.machine, item.date]);
...
PerformaDetailContent.displayName = 'PerformaDetailContent';
...
const DetailModal = ({ show, onClose, item }) => {
    ...
    const renderDetailContent = () => {
        ...
        if (packageType === "PERFORMA RED AND GREEN") {
            return <PerformaDetailContent item={item} />;
        }
    ...
    return (
        <MainLayout>
            ...
            <main className="flex-1 bg-white px-8 pt-16 pb-8">
                ...
                {/* Confirmation Modal dengan userRole prop */}
                <ConfirmModal
