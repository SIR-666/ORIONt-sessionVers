...
const ReportPerformance = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
          const response = await fetch('/api/getAllPerformance');
    ...
    const handleExport = () => {
        const formattedData = filteredData.map((row) => {
            const { ID, Tanggal, ProductionTime, OperationTime, NPT, RunningTime, AvailableTime, Breakdown, Planned, ProcessWaiting, QualityLoss, SpeedLoss, DowntimeGroup, UnavailableTime } = row;
            ...
            return {
                ...
                Shift: Shift === "I" ? "I" : Shift === "II" ? "II" : Shift === "III" ? "III" : 0,
                Group: DowntimeGroup,
                  ...
                  {loading ? (
                    ...
                    filteredData.map((row) => (
                      <tr className="bg-white border-b" key={row.ID}>
                        ...
                        <td className="py-4 px-6">{handleGroup(row.DowntimeGroup)}</td>
                        ...
                        <td className="py-4 px-6">
                          {row.Planned ? parseFloat(row.Planned).toFixed(3) : "N/A"}
                        </td>
                        <td className="py-4 px-6">
                          {row.Breakdown ? parseFloat(row.Breakdown).toFixed(3) : "N/A"}
                        </td>
                        <td className="py-4 px-6">
                          {row.ProcessWaiting ? parseFloat(row.ProcessWaiting).toFixed(3) : "N/A"}
                        </td>
                        <td className="py-4 px-6">
                          {row.QualityLoss
