...
export async function POST(req){
    const { machine, code, startTime, endTime, duration, comments, shift, line, type, group } = await req.json();
    console.log("Received data: \n", machine, code, startTime, endTime, duration, comments, shift, line, type, group);
    try {
      if (!machine || !startTime || !endTime || !duration || !comments || !shift || !line || !type || !group) {
        return new Response(
      ...
      const rowsAffected = await createStoppage({ machine, code, startTime, endTime, duration, comments, shift, line, type, group });
