    ...
    const fetchDownTimeData = async () => {
      try {
        ...
        initialData.forEach(async (entry) => {
          ...
          let endTime = entry.actual_end ? new Date(entry.actual_end) : new Date();
          const startUTCTime = new Date(startTime.setHours(startTime.getHours() - 7));
          const endUTCTime = new Date(endTime.setHours(endTime.getHours() - 7));

          start = startUTCTime < shiftTimes.startTime ? shiftTimes.startTime : shiftTimes.startTime;
          end = endUTCTime < shiftTimes.endTime ? shiftTimes.endTime : shiftTimes.endTime;
          console.log("Sent data for chart: ", toLocalISO(start), toLocalISO(end) || localTime, value);
          const response = await fetch('/api/getDowntimePO', {
            method: 'POST',
            headers: {
            'Content-Type': 'application/json',
            },
            body: JSON.stringify({ date_start: toLocalISO(start), date_end: toLocalISO(end) || localTime, line: value })
          });
...
initialData.forEach((order) => {
  ...
  const shiftPOs = allPO.filter((po) => {
    ...
    return (
      (poStart >= getShiftTimes.startTime && poStart < getShiftTimes.endTime) || // PO starts within shift
  ...
  const sortedShiftPOs = shiftPOs.sort((a, b) => new Date(a.actual_start) - new Date(b.actual_start));
  ...
  const hasDowntimeAfterLastPO = downtimeData.some((downtime) => {
    ...
    downtimeEnd.setHours(downtimeEnd.getHours() - 7);
    return (
      (downtimeStart >= previousPOEnd && downtimeStart < getShiftTimes.endTime) || 
  ...
  const filteredDowntimes = (Array.isArray(downtimeData) ? downtimeData : []).filter((downtime) => {
    downtimeEndTime = calculateEndTime(downtime.Date, downtime.Minutes);
    
    return (
      downtime.Line.toUpperCase() === line.toUpperCase() &&
      (
        (downtime.Date <= toLocalISO(getShiftTimes.endTime) && downtimeEndTime >= toLocalISO(getShiftTimes.startTime)) // Overlaps with order's range
      )
    );
  });

  const downtimesWithUnavailabilities = [...filteredDowntimes, ...unavailableTimes];
  ...
  if (groupedOrdersMap.has(id)) {
    ...
    existingOrder.downtimes = existingOrder.downtimes.sort((a, b) => new Date(a.Date) - new Date(b.Date));
    existingOrder.orderEndTime = new Date(Math.max(new Date(existingOrder.orderEndTime), new Date(orderEndTime)));
  } else {
    groupedOrdersMap.set(id, { ...order, downtimes: sortedDowntimes, orderEndTime });
  }
...
const calculateDurationPercentage = (startTime, endTime, startDate, endDate) => {
...
const parseISOToLocal = (isoString) => {
  ...
  if (!year || !month || !day || !hours || !minutes || !seconds) {
    throw new Error(`Invalid date or time components in ISO string: ${isoString}`);
  }
...
const renderLegend = () => (
  <div style={{ display: 'flex', justifyContent: 'space-around', marginBottom: '10px', width: '100%' }}>
    <div style={{ display: 'flex', alignItems: 'center' }} aria-label="Runtime">
      <div style={{ width: '20px', height: '10px', backgroundColor: '#4CAF50', marginRight: '5px' }}></div>
      <span className="text-black">Runtime</span>
    </div>
    <div style={{ display: 'flex', alignItems: 'center' }} aria-label="Planned Stoppages">
      <div style={{ width: '20px', height: '10px', backgroundColor: '#FFD700', marginRight: '5px' }}></div>
      <span className="text-black">Planned Stoppages</span>
    </div>
    <div style={{ display: 'flex', alignItems: 'center' }} aria-label="Breakdown">
      <div style={{ width: '20px', height: '10px', backgroundColor: '#FF6347', marginRight: '5px' }}></div>
      <span className="text-black">Breakdown/Process Failure/Minor Stop</span>
    </div>
    <div style={{ display: 'flex', alignItems: 'center' }} aria-label="Unavailable Time">
      <div style={{ width: '20px', height: '10px', backgroundColor: '#CDA2AB', marginRight: '5px' }}></div>
      <span className="text-black">Unavailable Time</span>
    </div>
    <div style={{ display: 'flex', alignItems: 'center' }} aria-label="Process Waiting">
      <div style={{ width: '20px', height: '10px', backgroundColor: '#E92CF9', marginRight: '5px' }}></div>
...
const renderOrders = () => {
  ...
  const calculateShiftBoundaries = (shift, referenceDate) => {
    ...
    const [startHours, startMinutes, startSeconds] = shiftStartTimes[shift].split(":");
    const [endHours, endMinutes, endSeconds] = shiftEndTimes[shift].split(":");
  ...
  return sortedOrders.map((order, index) => {
    ...
    const { start: shiftStart, end: shiftEnd } = calculateShiftBoundaries(renderedShift, shiftTimes.startTime);
    ...
    if (!downtimes || downtimes.length === 0) {
      return (
        <div key={id} style={{ position: 'relative', marginBottom: '20px', marginLeft: '10px', width: '100%' }}>
          <div
            style={{
              position: 'relative',
              height: '20px',
        ...
        >
          {downtimes.map((downtime) => {
            ...
            if (downtime.Downtime_Category === 'Planned Stop') {
              barColor = '#FFD700'; 
            } else if (['Breakdown', 'Minor Stop', 'Process Failure'].includes(downtime.Downtime_Category)) {
              barColor = '#FF6347'; 
            ...
            return (
              <div
                ...
                title={`Category: ${downtime.Downtime_Category}\nStart: ${formatDateTime(downtime.Date)}\nEnd: ${formatDateTime(calculateEndTime(
                  downtime.Date,
                  downtime.Minutes
          ...
          {endTime > localTime && (
            <div
              style={{
                position: 'absolute',
                left: `${calculateStartPercentage(endTime, actual_start, actual_end || localTime)}%`,
                width: `${100 - calculateStartPercentage(endTime, actual_start, actual_end || localTime)}%`,
